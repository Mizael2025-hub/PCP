...
export type OxideMillProductionTableProps = {
  data: OxideMillProductionWithRelations[]
  shifts: Shift[]
  filters: {
    dateFrom: string
    dateTo: string
  ...
  onEdit: (record: OxideMillProductionWithRelations) => void
}
...
export function OxideMillProductionTable({
  data,
  shifts,
  filters,
  onEdit
}: OxideMillProductionTableProps) {
  ...
  }, [filters.dateFrom, filters.dateTo, filters.shiftId])
  ...
  const columns = useMemo<ColumnDef<OxideMillProductionWithRelations>[]>(
    () => [
      {
        accessorKey: "date",
        header: "Data",
      ...
      {
        accessorKey: "oxidation_degree",
        header: "Grau oxidação (%)",
        cell: ({ getValue }) => (
          <span className="tabular-nums text-zinc-700 dark:text-zinc-300">
            {formatDegree(getValue<number>())}
          </span>
        )
      },
      {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const record = row.original
      const searchable = [record.shifts?.name, record.employees?.name]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
  ...
  function applyFilters() {
    ...
    startTransition(() => {
      router.push(
        query ? `/producao/oxide-mill?${query}` : "/producao/oxide-mill"
      )
    })
  }
  ...
  function clearFilters() {
    startTransition(() => {
      router.push("/producao/oxide-mill")
    })
  }
      ...
      toolbar={
        <div className="flex flex-col gap-3">
          ...
          <div className="relative max-w-sm">
            ...
            />
            <Input
              placeholder="Buscar por operador ou turno..."
              value={globalFilter}
