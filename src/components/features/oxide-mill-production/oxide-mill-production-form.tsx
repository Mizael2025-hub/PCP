...
import {
  oxideMillFormSchema,
  type OxideMillFormSchema
} from "@/validations/oxide-mill-production/production-schema"

export type OxideMillProductionFormProps = {
...
export function OxideMillProductionForm({
  record,
  shifts,
  employees,
  onSuccess,
  onCancel
}: OxideMillProductionFormProps) {
  ...
  } = useForm<OxideMillFormSchema>({
    resolver: zodResolver(oxideMillFormSchema),
    defaultValues: {
      date: record?.date ?? today,
      shift_id: record?.shift_id ?? "",
      operator_id: record?.operator_id ?? "",
      oxide_weight: record?.oxide_weight ?? ("" as unknown as number),
      oxidation_degree: record?.oxidation_degree ?? ("" as unknown as number)
    }
  })
  ...
  async function onSubmit(data: OxideMillFormSchema) {
    try {
      const payload = {
        date: data.date,
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input
          ...
          placeholder="0,00"
          disabled={isSubmitting}
          error={errors.oxidation_degree?.message}
          className="tabular-nums"
          {...register("oxidation_degree")}
        />
      </div>
