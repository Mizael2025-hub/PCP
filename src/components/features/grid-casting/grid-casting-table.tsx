...
export type GridCastingTableProps = {
  data: GridCastingProductionWithRelations[]
  shifts: Shift[]
  filters: {
    dateFrom: string
    dateTo: string
  ...
  onEdit: (record: GridCastingProductionWithRelations) => void
}
...
export function GridCastingTable({
  data,
  shifts,
  filters,
  onEdit
}: GridCastingTableProps) {
  ...
  const columns = useMemo<ColumnDef<GridCastingProductionWithRelations>[]>(
    () => [
      {
        accessorKey: "date",
        header: "Data",
      ...
      {
        id: "battery_model",
        accessorFn: (row) => row.battery_models?.code ?? "",
        header: "Modelo",
        cell: ({ row }) => (
          <span className="tabular-nums text-zinc-600 dark:text-zinc-400">
            {row.original.battery_models?.code ?? "—"}
          </span>
        )
      },
      {
  ...
  function clearFilters() {
    startTransition(() => {
      router.push("/producao/grid-casting")
    })
  }
      ...
      toolbar={
        <div className="flex flex-col gap-3">
          ...
          <div className="relative max-w-sm">
            ...
            />
            <Input
              placeholder="Buscar por máquina, operador ou modelo..."
              value={globalFilter}
