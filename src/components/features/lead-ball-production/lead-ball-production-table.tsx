...
export type LeadBallProductionTableProps = {
  data: LeadBallProductionWithRelations[]
  shifts: Shift[]
  filters: {
    dateFrom: string
    dateTo: string
  ...
  onEdit: (record: LeadBallProductionWithRelations) => void
}

const PAGE_SIZE_OPTIONS = [5, 10, 20] as const
const ALL_SHIFTS_VALUE = "all"
const ALL_SILOS_VALUE = "all"
...
export function LeadBallProductionTable({
  data,
  shifts,
  filters,
  onEdit
}: LeadBallProductionTableProps) {
  ...
  const [siloNumber, setSiloNumber] = useState(
    filters.siloNumber || ALL_SILOS_VALUE
  )
  ...
  useEffect(() => {
    ...
    setSiloNumber(filters.siloNumber || ALL_SILOS_VALUE)
  }, [filters.dateFrom, filters.dateTo, filters.shiftId, filters.siloNumber])
  ...
  const siloFilterOptions = useMemo(() => {
    const silos = new Set(data.map((record) => record.silo_number))
    const sorted = Array.from(silos).sort((a, b) => a - b)

    return [
      { value: ALL_SILOS_VALUE, label: "Todos os silos" },
      ...sorted.map((silo) => ({
        value: String(silo),
        label: `Silo ${silo}`
      }))
  ...
  const columns = useMemo<ColumnDef<LeadBallProductionWithRelations>[]>(
    () => [
      {
        accessorKey: "date",
        header: "Data",
  ...
  function applyFilters() {
    ...
    if (siloNumber && siloNumber !== ALL_SILOS_VALUE) {
      params.set("siloNumber", siloNumber)
    }
    ...
    startTransition(() => {
      router.push(
        query ? `/producao/lead-ball?${query}` : "/producao/lead-ball"
      )
    })
  }
  ...
  function clearFilters() {
    startTransition(() => {
      router.push("/producao/lead-ball")
    })
  }
  ...
  const filtersChanged =
    ...
    (siloNumber !== ALL_SILOS_VALUE ? siloNumber : "") !== filters.siloNumber
        ...
        <div className="flex flex-col gap-3">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Select
              ...
              value={siloNumber}
              onChange={(event) => setSiloNumber(event.target.value)}
              disabled={isPending}
            />
          ...
          <div className="relative max-w-sm">
            ...
            />
            <Input
              placeholder="Buscar por operador, turno ou silo..."
              value={globalFilter}
